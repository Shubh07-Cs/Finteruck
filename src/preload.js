...
try {
  contextBridge.exposeInMainWorld('electronAPI', {
    ...
    analyzeMultimodal: (audioBase64) => {
      console.log('PreloadAPI: analyzeMultimodal called');
      return ipcRenderer.invoke('analyze-multimodal', audioBase64).catch(err => {
        console.error('PreloadAPI: analyzeMultimodal error:', err);
        return { error: err.message };
      });
    },
    ...
    onTriggerAnalyze: (callback) => {
      ...
      ipcRenderer.on('trigger-analyze', handler);
      return () => ipcRenderer.removeListener('trigger-analyze', handler);
    },

    onToggleVoiceRecognition: (callback) => {
      const handler = () => callback();
      ipcRenderer.on('toggle-voice-recognition', handler);
      return () => ipcRenderer.removeListener('toggle-voice-recognition', handler);
    },
